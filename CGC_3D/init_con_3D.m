function [initSG,seamnode,costmap,labelmap,strucmap,condierr] = init_con_3D(rr,TI,conditioning,filename)

global nbreplicates patchsize overlap
global indexmap SGdim TIdim w_c

indcond = sub2ind(SGdim,conditioning(:,1),conditioning(:,2),conditioning(:,3));
cond = nan(SGdim); cond(indcond)=conditioning(:,4);

initSG = cond;    
costmap = zeros(SGdim); labelmap = nan(SGdim);
...
for kk = 1:offset(3):SGdim(3)-overlap(3)
    for jj = 1:offset(2):SGdim(2)-overlap(2)
        for ii = 1:offset(1):SGdim(1)-overlap(1)
            ...
            consim = cond(ii:maxx,jj:maxy,kk:maxz);
            simold(isfinite(consim)) = nan;
            a = sum(sum(sum(isfinite(simold))));
            b = sum(sum(sum(isfinite(consim))));            
            ...
            if ii*jj*kk == 1
                ...
                conold = consim(1:patchx,1:patchy,1:patchz);
...
%                dis = dis3D(TIs,realold);
                dis_1 = dis3D(TI,simold);
                if b ~= 0
                    dis_2 = dis3D(TI,consim);
                else
                    dis_2 = zeros(size(dis_1));
                end
...
initnamei = ['con_init',filename,num2str(rr),'.SGEMS'];
WriteGrid(initSG,initnamei,'con_init');
WriteGrid(strucmap,['con_init-struct-',num2str(rr),'.SGEMS'],'init');
end
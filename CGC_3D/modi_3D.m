function [modiSG,labelmap,strucmap,ch] = modi_3D(rr,TI,initSG,seamnode,costmap,labelmap,strucmap,filename)

% the function to modify the initial 3D realization
global nbreplicates stop maxloop
global indexmap SGdim TIdim halfpr
...
while (l < maxloop && meanerr>stop)
    ...
    realold = initSG(SGpickx:patchmaxx,SGpicky:patchmaxy,SGpickz:patchmaxz);
...
%    TIs = TI(1:TIdim(1)-patchsizex+1,1:TIdim(2)-patchsizey+1,1:TIdim(3)-patchsizez+1);
    ...
    if sum(newseamnode(:,4)) <= sum(oldseamnode(:,4))
        ...
        initSG(SGpickx:patchmaxx,SGpicky:patchmaxy,SGpickz:patchmaxz) = realprop;
...
figure(3); clf;
plot([1:size(ch,1)],ch);
saveas(gcf,['err-change',num2str(rr),'.fig']);
